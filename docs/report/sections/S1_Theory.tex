...
\texttt{problem/geometry/geometry_types/cylinder.py} &
Implemented \
...
\texttt{mechanics/kinematics/small_strain.py} &
Implemented \

4 &
Implement isotropic linear elasticity &
\texttt{mechanics/constitutive/linear_elastic.py} &
Implemented \

5 &
Define the compression boundary constraints &
...
8 &
Define numerical loading increments from the problem-level loading path &
\texttt{problem/definition.py},
future numerical stepping layer &
Next \

9 &
Solve the linear elasticity system with DOLFINx/PETSc &
\texttt{numerical/solver/} &
TODO \
...
\texttt{benchmarks/01_cylinder_compression/reference.py} &
TODO \

14 &
Compare the finite-element solution with the reference solution &
\texttt{benchmarks/01_cylinder_compression/},
\texttt{tests/} &
TODO \

15 &
Perform a mesh-convergence study &
\texttt{benchmarks/01_cylinder_compression/experiment.yaml},
\texttt{tests/} &
TODO \
...
\texttt{mechanics/kinematics/finite_strain.py} &
TODO \

18 &
Implement a compressible Neo-Hookean constitutive model &
\texttt{mechanics/constitutive/neo_hookean.py} &
TODO \
...
The immediate milestone is to complete the small-strain linear-elastic
solution path:
...
\text{Dirichlet BCs}
\rightarrow
\text{Loading increments}
\rightarrow
\text{Linear solver}.
$$

This first executable benchmark intentionally uses prescribed displacement
on the top and bottom surfaces rather than explicit platen contact. The
lateral surface remains traction-free through the natural boundary condition
of the variational formulation. This provides a simple displacement-controlled
compression problem against which the numerical formulation and solver can
first be validated.

Frictionless unilateral contact is therefore treated as a later numerical
extension rather than a prerequisite for the initial benchmark.

The finite-strain and contact developments are kept separate from the
initial linear-elastic solution so that each extension can be validated
against the already established small-strain formulation.
